...
class BusinessSettingService extends BaseService implements BusinessSettingServiceInterface
{
    ...
    {
        $APP_VERSION = $this->businessSettingRepository->findOneBy(criteria: [
            'settings_type' => APP_VERSION,
            'key_name' => APP_VERSION
        ]);
        if ($APP_VERSION) {
            $this->businessSettingRepository->update(id: $APP_VERSION->id, data: ['key_name' => APP_VERSION, 'settings_type' => APP_VERSION, 'value' => $data]);
        } else {
            $this->businessSettingRepository->create(data: ['key_name' => APP_VERSION, 'settings_type' => APP_VERSION, 'value' => $data]);
        }
    }
