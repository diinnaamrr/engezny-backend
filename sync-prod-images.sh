...
# Run this script on the server: /var/www/engezny-prod
# ===========================================

echo "📋 Copying images from Docker container to production server..."

cd /var/www/engezny-prod
...
docker cp engezny_app_prod:/var/www/html/public/landing-page/assets/img/clients/. ./public/landing-page/assets/img/clients/ 2>/dev/null || {
    echo "⚠️  Container may not be running. Trying to copy from image directly..."
    
    # محاولة نسخ من image مباشرة
    TEMP_CONTAINER=$(docker create ghcr.io/diinnaamrr/engezny-backend:latest 2>/dev/null || echo "")
